...
const chartOptions = computed<ApexOptions>(() => ({
    ...
    colors: selectedSensors.value.map(
        (key) => sensorOptions.find((s) => s.key === key)?.color ?? '#94a3b8',
    ),
        ...
        >
            ...
            >
                ...
                <button
                    id="btn-export-csv"
                    @click="exportCsv"
                    class="flex items-center gap-2 rounded-xl border border-border/50 bg-card/60 px-4 py-2 text-sm font-medium text-muted-foreground backdrop-blur-md transition-all hover:border-primary/30 hover:bg-primary/10 hover:text-primary"
                >
            ...
            >
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    <span
                        class="self-center text-xs font-medium tracking-wider text-muted-foreground uppercase"
                        >Sensors:</span
                    >
                    <label
                        ...
                        :style="
                            selectedSensors.includes(opt.key)
                                ? `background-color: ${opt.color}33; color: ${opt.color}; border-color: ${opt.color}66`
                                : ''
                        "
                    >
                        <input
            ...
            >
                ...
                <div
                    v-else-if="!readings?.data.length"
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            ...
                            class="flex items-center gap-1 rounded-lg border border-border/50 px-3 py-1.5 text-sm text-muted-foreground transition-all hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                        ...
                        <button
                            ...
                            @click="goToPage(readings!.current_page + 1)"
                            :disabled="
                                readings.current_page >= readings.last_page
                            "
                            class="flex items-center gap-1 rounded-lg border border-border/50 px-3 py-1.5 text-sm text-muted-foreground transition-all hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-40"
                        >
